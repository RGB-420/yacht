...
class FalmouthClassicsScraper(BaseScraper):

    def __init__(self):
        super().__init__("falmouth_classics")
    ...
    def get_column_indices(self, tabla):
        ...
        self.logger.info(f"[INFO] Detected headers: {list(col_index.keys())}")
...
def scrape(url, browser):
    scraper = FalmouthClassicsScraper()
