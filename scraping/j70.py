import pandas as pd
from bs4 import BeautifulSoup
import numpy as np
from rapidfuzz import process, fuzz

import re


rank_words = ['Rank', 'Pos']

columns_words = {
    "owner": ["owner"],
    "boat": ["yacht", "boat"],
    "number": ["sail number", "sailno", "number"]
}

def scrape(url, browser):
    df = pd.DataFrame()

    page = browser.new_page()
    links = get_links(page, url)

    for link in links:
        html = get_html_with_playwright(page, link)
        if html:
            final_page = get_index_page(html)
            if final_page:
                headers = get_columns(html)
                if headers:
                    column_map = get_column_mapping(headers)

                    datos = obtener_indice(html, column_map)

                    df_index = pd.DataFrame(datos)

            else:
                headers = get_columns(html)
                if not headers:
                    print("❌ No se detectaron columnas")
                    continue

                column_map = get_column_mapping(headers)

                datos = obtener_barcos(html, column_map)

                df_temp = pd.DataFrame(datos)
                df = pd.concat([df, df_temp], ignore_index=True)
    

    df_clean = normalize_data(df, df_index)
    
    return df_clean[['boat', 'number', 'owner']]



def get_index_page(html):
    soup = BeautifulSoup(html, "html.parser")

    tabla = soup.find("table").find("tbody")
    if not tabla:
        print("❌ No se encontró la tabla")
        return

    rows = tabla.find_all("tr")

    if not rows:
        print("❌ La tabla no tiene filas")
        return

    first_row = rows[0]

    cell = first_row.find("td", class_="s0")
    if not cell:
        print("❌ La fila no tiene celdas")
        return

    title = cell.get_text(strip=True)
    
    final_word = "final"

    final_results = final_word.lower() in title.lower()

    return final_results

def get_links(page, url):
    page.goto(url)
    #page.wait_for_load_state("networkidle")
    page.wait_for_timeout(3000)

    links = []

    iframe_srcs = page.eval_on_selector_all(
        "iframe",
        "els => els.map(e => e.src).filter(src => src)"
    )

    for iframe in iframe_srcs:
        if re.match(r"^https://docs\.google\.com/spreadsheets", iframe):
            print("Es un link de Google Sheets")
            links.append(iframe)
        else:
            print("No es un link", iframe)

    return links

def get_html_with_playwright(page, url):
    page.goto(url)
    page.wait_for_timeout(1500)
    html = page.content()
    return html

def get_columns(html):
    soup = BeautifulSoup(html, "html.parser")
    tabla = soup.find("table", class_="waffle")
    if not tabla:
        return None

    for row in tabla.find_all("tr"):
        cells = row.find_all("td")
        if not cells:
            continue

        headers = [cell.get_text(strip=True) for cell in cells]
        joined = " ".join(h.lower() for h in headers)

        if any(word.lower() in joined for word in rank_words):
            return headers

    return None

def get_column_mapping(headers):
    mapping = {key: None for key in columns_words}

    for idx, header in enumerate(headers, start=0):
        header_norm = header.lower().strip()

        for column_key, keywords in columns_words.items():
            if any(word in header_norm for word in keywords):
                mapping[column_key] = idx

    return mapping

def obtener_barcos(html, column_map):
    soup = BeautifulSoup(html, "html.parser")
    tabla= soup.find("table")
    datos = []

    filas = tabla.find("tbody").find_all("tr")

    for fila in filas[2:]:
        datos_barcos = fila.find_all("td")

        def get_value(idx):
            if idx is None or idx >= len(datos_barcos):
                return None
            return datos_barcos[idx].get_text(strip=True)


        fila_data = {
            "boat": get_value(column_map["boat"]),
            "number": get_value(column_map["number"]),
            "owner": get_value(column_map["owner"])
        }

        datos.append(fila_data)

    return datos

def obtener_indice(html, column_map):
    soup = BeautifulSoup(html, "html.parser")
    tabla= soup.find("table")
    datos = []

    filas = tabla.find("tbody").find_all("tr")

    for fila in filas[3:55]:
        datos_barcos = fila.find_all("td")

        def get_value(idx):
            if idx is None or idx >= len(datos_barcos):
                return None
            return datos_barcos[idx].get_text(strip=True)

        fila_data = {
            "boat": get_value(column_map["boat"])
        }

        datos.append(fila_data)

    return datos


def normalize_data(df, df_index):
    df_clean = df.copy()

    df_clean["boat_original"] = df_clean["boat"]
    df_clean["boat"] = df_clean["boat"].apply(normalize_boat_name)
    df_clean["boat_norm"] = df_clean["boat"]

    df_clean["number"] = df_clean["number"].apply(normalize_sail_number)

    df_index["boat_norm"] = df_index["boat"].apply(normalize_boat_name)
    index_names = df_index["boat_norm"].dropna().unique().tolist()

    df_clean = df_clean[
        df_clean["boat"].notna() &
        ~df_clean["boat"].str.contains(
            r"MIXED|YOUTH|OPEN|CORINTHIAN|YACHT|GS#|CATEGORY",
            case=False,
            na=False
        )
    ]
    
    matches = df_clean["boat_norm"].apply(
        lambda x: fuzzy_match_boat(x, index_names)
    )

    df_clean["boat_match"] = matches.apply(lambda x: x[0] if x else None)
    df_clean["match_score"] = matches.apply(lambda x: x[1] if x else 0)

    df_clean["boat"] = np.where(
        df_clean["boat_match"].notna(),
        df_clean["boat_match"],
        df_clean["boat_original"]
    )

    df_clean = df_clean[df_clean["match_score"] >= 85]

    df_clean = (
        df_clean
        .sort_values("match_score", ascending=False)
        .drop_duplicates(subset=["boat"], keep="first")
    )

    return df_clean


def normalize_boat_name(name):
    if pd.isna(name):
        return np.nan
    name = str(name).upper().strip()
    name = name.replace("/", " ")
    name = re.sub(r"[^A-Z0-9 ]", "", name)
    name = re.sub(r"\s+", " ", name)
    return name

def normalize_sail_number(x):
    if pd.isna(x):
        return np.nan
    x = str(x).upper().replace(" ", "")
    if re.fullmatch(r"\d+", x):
        return f"GBR{x}"
    return x

def fuzzy_match_boat(name, index_names, threshold=85):
    if pd.isna(name):
        return None, 0

    match, score, _ = process.extractOne(
        name,
        index_names,
        scorer=fuzz.token_sort_ratio
    )

    if score >= threshold:
        return match, score
    return None, score
